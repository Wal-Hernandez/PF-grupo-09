...
const postReview = async (req, res) => {
  try {
    const { userId, businessId, score, title, comment, mail } = req.body;

    if (!userId || !businessId || !score || !title || !comment || !mail) res.status(400).json({ msg: "You need to fill all the fields to leave a review" });
