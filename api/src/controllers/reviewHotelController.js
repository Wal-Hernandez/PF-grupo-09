...
const postReview = async (req, res) => {
  try {
    const { userId, hotelId, score, title, comment, mail } = req.body;

    if (!userId || !hotelId || !score || !title || !comment || !mail) res.status(400).json({ msg: "You need to fill all the fields to leave a review" });
