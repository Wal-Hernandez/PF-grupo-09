...
export default function Details({ userlog }) {
  ...
  packageDetail.activities?.map((e) => {
    packageActivity.push(<div>{e.name}</div>);
  });
  ...
  useEffect(() => {
    ...
    //dispatch(getClean());
    if (user?.email) dispatch(loadCart(user?.email));
  }, [dispatch, id, user]);
  ...
  let arrayCartLoggedin = useSelector(
    (state) => state.rootReducer.arrayCartLoggedin
  );
  ...
  if (localStorage.getItem("myCartNotLoggedin")) {
    ...
    myCartparsedfilteredNotLoggedin = myCartparsedNotLoggedin.filter(
      (p) => p.id == id
    );
  ...
  if (user) {
    if (localStorage.getItem("myCartLoggedin")) {
      ...
      myCartparsedfilteredLoggedin = myCartparsedLoggedin.filter(
        (p) => p.id == id
      );
  ...
  if (cart[0]) {
    if (cart[0].cartDetails.length > 0) {
      let res = cart[0].cartDetails.filter(
        (d) => d.packageId == packageDetail.id
      );
  ...
  if (localStorage.getItem("myCartNotLoggedin")) {
    ...
    if (id) {
      console.log(myCartparsedNotLoggedin);
      let myCartparsedNotLoggedinfiltered = myCartparsedNotLoggedin.filter(
        (e) => e.id == id
      );
   ...
   {packageDetail.activities?.map(e => {
    return (<>
      ...
      <div className="detalles-div">
        ...
        !packageDetail.activities?.map((e) => e.reviewActivities).flat()
          .length ? (
          <p>Aún no hay valoraciones para los elementos de este paquete</p>
        ) : (
          <div className="review-container">
            <button
              class="btn-detalles-pag"
            ...
            {packageDetail.hotel?.reviewHotels?.length ? (
              <div>
                {showReviews && (
                  <ShowReviews
                    data={packageDetail.hotel?.reviewHotels}
                    titulo={`Hotel - ${packageDetail.hotel?.name}`}
                  />
                )}
              </div>
            ) : null}
            {packageDetail.business?.reviewBusinesses?.length ? (
              <div>
                {showReviews && (
                  <ShowReviews
                    data={packageDetail.business?.reviewBusinesses}
                    titulo={`Transporte - ${packageDetail.business?.name}`}
                  />
                )}
              </div>
            ) : null}
            {packageDetail.activities?.map((act) =>
              act?.reviewActivities.length ? (
                <div>
                  {showReviews && (
                    <ShowReviews
                      data={act?.reviewActivities}
                      titulo={`Actividad - ${act.name}`}
                    />
                  )}
                </div>
              ) : null
